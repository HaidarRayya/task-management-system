...
class TaskService
{
    ...
    public function allTasks($status, $priority, $employee_id, $type, $due_date, $sort)
    {
        try {
            if (AuthService::user_role(Auth::user()->id) == UserRole::ADMIN->value) {
                $tasks = Cache::remember('tasks', 3600, function () use ($status, $priority, $employee_id, $type, $due_date, $sort) {
                    return  Task::query()->with(['employee' => function ($q) {
                ...
                $tasks = Cache::remember('tasks', 3600, function () use ($status, $priority, $due_date, $sort) {
                    return Task::myTask(Auth::user()->id)->byPriority($priority)
                        ->byStatus($status)
                        ->byDueDate($due_date)
                        ->sortDueDate($sort)
                        ->get();
    ...
    public function oneTask($task_id)
    {
        try {
            $task = Cache::remember('task_' . $task_id, 600, function () use ($task_id) {
                return Task::findOrFail($task_id)->load('employee.role');
            });
            $task = TaskResource::make($task);
            return $task;
        } catch (Exception $e) {
    ...
    {
        try {
            $tasks = Cache::remember('deleted_tasks', 3600, function () use ($status, $priority, $employee_id, $type, $due_date, $sort) {
    ...
    public function assignEmployee($taskData, $task_id)
    {
        ...
        if (($task->status == TaskStatus::OPEN->value) && (AuthService::user_role($taskData['employee_id']) == UserRole::TESTER->value)) {
            throw new HttpResponseException(response()->json(
                [
                    'status' => 'error',
     ...
     * @param int $task_id      
     */
    public function startWorkTask($task_id)
    {
        $task = Task::findOrFail($task_id);
     ...
     * @param int $task_id      
     */
    public function endWorkTask($task_id)
    {
        $task = Task::findOrFail($task_id);
     ...
     * @param int $task_id      
     */
    public function startTestTask($task_id)
    {
        $task = Task::findOrFail($task_id);
     ...
     * @param int $task_id      
     */
    public function endTestTask($task_id)
    {
        $task = Task::findOrFail($task_id);
    ...
    {
        $reports = Report::whereDate('created_at', Carbon::today())->with('task')->get();
